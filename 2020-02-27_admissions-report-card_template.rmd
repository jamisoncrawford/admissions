...
Effectiveness ([**OIE**](https://oie.gsu.edu/)) excludes certificate-seekers, 
non-degree students in graduate-level courses, and transient students. 
...
The following visualizes the **`r college`'s** graduate-level and degree-seeking 
target enrollees, historical enrollment, and linear trendlines by academic term 
from **FY `r min_fy`** to **FY `r max_fy`**. 
...
*Admissions Report Card* are reproducible in their entirety, from raw data to
the present data product, using scripts located in this 
