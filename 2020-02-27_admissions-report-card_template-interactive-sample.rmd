---
title: "Admissions Report (Sample Work Product)"
author: "J. Crawford, Institutional Researcher"
...
setwd("~/R Portfolio/Admission Report")
...
all <- readRDS("app-data-scrambled.RDATA")
...
This sample work product is a dynamic R Markdown report designed to provide an 
overview of historical admissions data for each college at 
[**Georgia State University**](https://www.gsu.edu/), as well as predicted 
applications and recommended enrollment rates over the next two fiscal years. 
...
Reproducible documentation for this work is available in this 
[**GitHub repository**](https://github.com/jamisoncrawford/admissions).

**Note:** *All data have been randomly transformed using the original admissions 
data in order to obfuscate protected GSU data while allowing sensible trends in 
seasonality and cyclicity that reasonably resemble actual admissions data.*
...
This report is programmed in R, Markdown, HTML, and CSS to provide a flexible 
template that reacts and updates dynamically when introduced to new data or 
filtered to show different colleges. All characters in **boldface** indicate 
reactive variables which change in accordance with the properties of the data.
...
See the reproducible documentation provided in the above hyperlink to view the 
underlying code.
...
While all tables were compiled with HTML and offer some interactivity, each data 
visualization was created using R package `ggplot2` and JavaScript library 
`plotly`, allowing both the visual advantage of the big picture and the rich 
granularity of traditional tabular data.

<br>

<center>

![](interactivity-gif.gif)
...
```{r out.width = "100%", out.height = "100%", fig.align = "center", dpi = 320}

# VISUALIZATION: ALL ENROLLMENT OVER TIME

all_fy_viz <- all_fy_viz %>%
...
viz <- ggplot(all_fy_viz, aes(x = FY, 
                       y = Applicants, 
                       fill = Status)) +
  geom_area(alpha = 0.6) +
...
title <- "Admissions by Decision"

subtitle <- paste0(college, ", Fiscal Years ", min_fy, " - ", max_fy)
...
ggplotly(viz) %>%
  layout(legend = list(orientation = "h",
         ...
         annotations = list(list(x = 0.0225, 
                                 y = 1.15, 
                                 text = title, 
                                 font = list(size = 20,
                                             face = "bold"),
                            ...
                            list(x = 0.976, 
                                 y = -0.125, 
                                 text = "Source: GSU Data Warehouse", 
                                 showarrow = F, 
                                 xref = 'paper', 
                                 yref = 'paper')))
...
viz <- ggplot(new_fy_viz, aes(x = FY, 
                       y = Enrollees)) +
...
title <- "Newly Enrolled v. Continuing"
...
nmd_fy_viz <- nmd_fy_viz %>%
  rename(FY = fy,
         Enrollees = count) %>%
  mutate(Status = factor(x = status, 
                         levels = c("Continuing", 
                                    "New"),
                         ordered = TRUE))

viz <- ggplot(nmd_fy_viz, aes(x = FY, 
                       y = Enrollees,
                       fill = Status, 
                    ordered = TRUE)) +
  geom_area(alpha = 0.5) +
...
pred_fy <- data.frame(fy = c(fy_ts_prep$fy[nrow(fy_ts_prep)] + 1, 
                             fy_ts_prep$fy[nrow(fy_ts_prep)] + 2))
...
# MANIPULATE FORECAST OBJECT INTERNALS

fore <- forecast(object = fit, h = 2)

fore$lower <- round(fore$lower)
fore$upper <- round(fore$upper)

max_y <- max(c(max(fore$x), max(fore$upper)))



# VISUALIZE AUTO-ARIMA

title <- paste0("Predicted FY '", abb_max_fy + 1, 
                " & '", abb_max_fy + 2,
                " Applications", sep = " ")

subtitle <- paste0("ARIMA Model ", sub, ", ", college)

plot_ly() %>% 
  add_lines(x = time(fy_ts), 
            y = fy_ts,
            color = I("black"), 
            name = "Observed") %>% 
  add_lines(x = time(fore$mean), 
            y = fore$mean, 
            color = I(alpha("#0039A6", 1)), 
            name = "Forecasted") %>%
    add_ribbons(x = time(fore$mean), 
              ymin = fore$lower[, 1], 
              ymax = fore$upper[, 1],
              color = I(alpha("#0071CE", 0.6)), 
              name = "80% Confidence") %>%
    add_ribbons(x = time(fore$mean), 
              ymin = fore$lower[, 2], 
              ymax = fore$upper[, 2],
              color = I(alpha("#97CAEB", 0.6)), 
              name = "95% Confidence") %>%
  layout(legend = list(orientation = "h",
                       x = 0, 
                       y = -0.051775, 
                       name = "Test",
                       bgcolor = alpha("#FFFFFF", 0.6)),
         margin = list(r = 12.5, t = 50, b = 50),
         yaxis = list(range = c(0, max_y),
                      tickformat = ","),
         xaxis = list(range = c(min_fy - 1, 
                                max_fy + 2)),
         annotations = list(list(x = 0.0225, 
                                 y = 1.15, 
                                 text = title, 
...
title <- paste0("Predicted FY '", abb_max_fy + 1, 
                " & '", abb_max_fy + 2,
                " Fall Applications", sep = " ")

subtitle <- paste0("ARIMA Model ", sub, ", ", college)

plot_ly() %>% 
  add_lines(x = time(fall_ts), 
            y = fall_ts,
            color = I("black"), 
            name = "Observed") %>% 
  add_lines(x = time(fore$mean), 
...
title <- paste0("Predicted FY '", abb_max_fy + 1, 
                " & '", abb_max_fy + 2,
                " Spring Applications", sep = " ")

subtitle <- paste0("ARIMA Model ", sub, ", ", college)

plot_ly() %>% 
  add_lines(x = time(spring_ts), 
            y = spring_ts,
            color = I("black"), 
            name = "Observed") %>% 
  add_lines(x = time(fore$mean), 
...
title <- paste0("Predicted FY '", abb_max_fy + 1, 
                " & '", abb_max_fy + 2,
                " Summer Applications", sep = " ")

subtitle <- paste0("ARIMA Model ", sub, ", ", college)

plot_ly() %>% 
  add_lines(x = time(summer_ts), 
            y = summer_ts,
            color = I("black"), 
            name = "Observed") %>% 
  add_lines(x = time(fore$mean), 
...
fore <- forecast(object = fit, h = 6)

fore$lower <- round(fore$lower)
fore$upper <- round(fore$upper)

max_y <- max(fore$x) * 1.15



# VISUALIZE AUTO-ARIMA

title <- "Predictions with Seasonality & Cyclicity"

subtitle <- paste0("ARIMA Model ", sub, ", ", college)

plot_ly() %>% 
  add_lines(x = time(all_ts), 
            y = all_ts,
            color = I("black"), 
            name = "Observed") %>% 
  add_lines(x = time(fore$mean), 
...
# New Enrollment Guide

**FY `r max_fy + 1`** enrollment recommendations for the **`r college`** are 
calculated via a 3% increase to a rolling average of the two most recent fiscal 
years with complete data, viz. **FY `r max_fy - 1`** and **FY `r max_fy`**. 
Moreover, recommendations use only OIE-reported enrollees, i.e. Masters and 
Doctoral students.

The following provides:

* Total **`r college`** enrollment recommendations for **FY `r max_fy + 1`** 
* Enrollment recommendations for **`r pred_table_final$Term[1]`**, 
**`r pred_table_final$Term[2]`**, and **`r pred_table_final$Term[3]`**
* Enrollment recommendations by academic term and program

<br>
<br>

## FY `r max_fy + 1` New Enrollment
...
Increasing this rolling average by 3% yields a total annual recommendation of 
...
title <- paste("Graduate & Degree-Earning Enrollees")

subtitle <- paste0("FY ", max_fy + 1, " Recommended Enrollees: ", 
                   comma(new_fy_enr_goal), ", ", college)

viz <- ggplot(viz_tbl, aes(x = FY, y = Enrolled, color = Status)) +
...
### Contribution-Based Guides

Given the two-year rolling average of proportional contributions by both 
**FY `r max_fy - 1`** and **FY `r max_fy`** terms, recommended enrollment 
...
title <- paste("Contributions by Term,")

subtitle <- paste0("FY ", max_fy + 1, " Recommended Enrollees: ", comma(new_fy_enr_goal), ", ", college)

viz <- ggplot(viz_tbl, aes(x = FY, y = Enrolled, color = Type)) +
    geom_smooth(alpha = 0.1,
              method = "lm", 
              color = "gray90") +
  geom_line(size = 1.05) +
...
Accounting for **FY `r max_fy + 1`** recommended enrollment, the above linear 
models depict by term: 
...
Recommended **FY `r max_fy + 1`** graduate-level enrollment for the 
**`r college`** are organized as follows:
...
recommended program enrollment of **`r comma(new_term_goals$contribution[1])`** 
...
plot_size <- ifelse(n_programs >= 10, 450, 
                    ifelse(n_programs == 2, 150, 
                           ifelse(n_programs == 1, 100,
                                  n_programs * 50)))
...
programs_viz <- programs_viz %>%
  rename(Program = program,
         Contribution = new_fy_contribution) %>%
  mutate(Contribution = round(Contribution),
         Program = reorder(Program, Contribution))

viz <- ggplot(programs_viz, aes(x = Program, y = Contribution)) +
  geom_bar(stat = "identity",
           fill = "#0071CE",
           alpha = 0.6) +
  scale_y_continuous(label = comma) + 
  labs(x = NULL,
       y = "Recommended Enrollees") +
  coord_flip() +
  theme_minimal()

ggplotly(viz) %>% layout(height = plot_size)
...
of programs in **FY `r max_fy + 1`**, increased by 3%, result in the following 
recommended program enrollment among 
